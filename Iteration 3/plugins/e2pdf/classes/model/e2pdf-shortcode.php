            ...
            if (!$this->helper->load('filter')->is_stream($pdf) && file_exists($pdf)) {
                ...
                if ($entry->get('ID')) {
                    ...
                    if ($output && $output == 'url') {
                        if ($esc_url_raw) {
                            $url = esc_url_raw(
                                    $this->helper->get_frontend_pdf_url($url_data, $site_url)
                            );
                        } else {
                            $url = esc_url(
                                    $this->helper->get_frontend_pdf_url($url_data, $site_url)
                            ...
                            if (isset($atts['show']) && $atts['show'] == 'label') {
                                if (isset($attributes[$attribute]) || isset($attributes['pa_' . $attribute])) {
                                    ...
                                    if ($wc_attribute && is_a($wc_attribute, 'WC_Product_Attribute')) {
                                        ...
                                        $post_meta = wc_attribute_label($attribute);
                                    }
                                }
            ...
            if (method_exists($this, $foreach_shortcode)) {
                ...
                if (is_array($data) && count($data) > 0) {
                    ...
                    foreach ($data as $data_key => $data_value) {
                        ...
                        $replace = array(
                            '[e2pdf-foreach-index]' => $index,
                            '[e2pdf-foreach-counter]' => $index + 1,
                            '[e2pdf-foreach-key]' => $data_key,
                            '[e2pdf-foreach-value]' => $data_value,
                            '[e2pdf-foreach-evenodd]' => $evenodd
                        );
                        $response[] = $this->e2pdf_foreach_index($atts, str_replace(array_keys($replace), $replace, $value), 1);
                        $index++;
                    }
                }
            }
                        ...
                        if (method_exists($this, $foreach_shortcode)) {
                            ...
                            if (is_array($data) && count($data) > 0) {
                                ...
                                foreach ($data as $data_key => $data_value) {
                                    ...
                                    $evenodd = $index % 2 == 0 ? '0' : '1';
                                    $replace = array(
                                        '[e2pdf-foreach-index-' . $foreach . ']' => $index,
                                        '[e2pdf-foreach-counter-' . $foreach . ']' => $index + 1,
                                        '[e2pdf-foreach-key-' . $foreach . ']' => $data_key,
                                        '[e2pdf-foreach-value-' . $foreach . ']' => $data_value,
                                        '[e2pdf-foreach-evenodd-' . $foreach . ']' => $evenodd
                                    );
                                    $response[] = $this->e2pdf_foreach_index($attributes, str_replace(array_keys($replace), $replace, $shortcode['5']), $foreach + 1);
                                    $index++;
                                }
                            }
                        }
